"""
Custom transformers for diabetes prediction preprocessing
"""
import numpy as np
import pandas as pd
...
class FeatureEngineering(BaseEstimator, TransformerMixin):
    """
    Creates domain-specific features from raw health metrics.
    
    Features created:
    - PregnancyRatio: Pregnancies normalized by age
    - RiskScore: Weighted combination of glucose, BMI, and age
    - InsulinEfficiency: Ratio of insulin to glucose
    - Glucose_BMI: Interaction between glucose and BMI
    - BMI_Age: Interaction between BMI and age
    """
    def __init__(self):
        self.epsilon = 1e-5
...
class WoEEncoding(BaseEstimator, TransformerMixin):
    """
    Weight of Evidence (WoE) encoding for selected features.
    
    WoE transforms categorical/binned features into continuous values 
    based on their predictive power for the target variable.
    """
    def __init__(self):
        self.feature_bins = {
...
class ColumnSelector(BaseEstimator, TransformerMixin):
    """
    Selects specific columns from the feature set.
    """
